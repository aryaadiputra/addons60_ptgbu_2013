...
class account_profit_loss_new_xls(report_xls):
    ...
    def generate_xls_report(self, parser, data, obj, wb):
        ...
        row_income_level3 = self.xls_row_template(cols_specs, ['Field Kosong','Field Kosong','Field Kosong','Account Name','Field Kosong','Field Kosong','Balance Income','Field Kosong'])
        ...
        row_expense_level3 = self.xls_row_template(cols_specs, ['Field Kosong','Field Kosong','Field Kosong','Account Name','Field Kosong','Field Kosong','Balance Expense','Field Kosong'])
        ...
        #-------------------------------
        total['tot_net'] = -1 * (total['tot_income'] + total['tot_expenses'])
        
        print "total['tot_net']------------------------------------>>",total['tot_income'], total['tot_expenses'], total['tot_net']
        #####
        ###INCOME###
        lv2_name    = ""
        lv2_total    = 0.0
        ...
        for a in parser.get_lines_another('expense'):
            ...
            if a['type'] == 'view' and a['level'] == 2:
                ...
                if lv2_name <> "":
                    ...
                    if lv2_name == 'OTHER INCOME / (EXPENSES)':
                        total_other_income_expense = -1 * lv2_total 
                        ws.write(row_count, 6, total_other_income_expense, row_bold_bottom_border_style)
                    else:    
                        ws.write(row_count, 6, lv2_total, row_bold_bottom_border_style)
                    row_count += 3
                    ...
                    if custom['custom_label'] <> "":
                        ...
                        if custom['custom_label'] == "Income / (Loss) from Operation - EBITDA":
                            ...
                            print "Total Operation", lv2_total
                            #custom_amount_total_ebitda = -(custom_amount_total)
                            ebitda = gross_profit - lv2_total
                            print "ebitda", ebitda
                            ws.write(row_count, 6,  ebitda, row_bold_right_style)
                        elif custom['custom_label'] == "Gross Profit":
                            gross_profit = total_income - total_cogs
                            print "total_income>>>>>>>>>>>>", total_income
                            print "total_cogs>>>>>>>>>>>>>>", total_cogs
                            print "gross_profit>>>>>>>>>>>>>>", gross_profit
                            ws.write(row_count, 6,  gross_profit, row_bold_right_style)
                        elif custom['custom_label'] == 'Net Income / (Loss) Before Tax - NIBT':
                            nibt = ebitda + total_other_income_expense
                            ws.write(row_count, 6,  nibt, row_bold_right_style)
            ...
            if a['type'] == 'view' and a['level'] == 3:
                ...
                total_cogs += lv2_total
                if a['name'] == 'OTHER INCOME' or a['name'] == 'OTHER EXPENSES':
                    self.xls_write_row(ws, a, data, parser, row_count, row_income_level3, style)
