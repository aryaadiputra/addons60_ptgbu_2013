...
class account_profit_loss_usd_xls(report_xls):
        ...
        if data['form']['currency_rate'] == 0:
            ...
            cols_specs = [
                    ...
                    ('Balance IDR', 1, 180, 'number',
                        lambda x, d, p: parser._sum_currency_amount_account(-x['balance_dual']))
            ]                 
            ...
            for w in parser.get_lines_another('income'):
                if w['level'] <> 2: 
                        ...
                        style = row_bold_style   
                        total['tot_income'] += w['balance_dual']
                        total['tot_income_idr'] += parser._sum_currency_amount_account(w['balance_dual'])
            ...
            row_count += 1
            for e in parser.get_lines():
                if parser._sum_currency_amount_account(e['balance_dual']):
                    if e['level'] <> 2:
                        style = row_normal_style
                    else:
                        style = row_bold_style
                        total['tot_expenses'] += e['balance_dual']   
                        total['tot_expenses_idr'] += parser._sum_currency_amount_account(e['balance_dual'])
                self.xls_write_row(ws, e, data, parser, row_count, row_balance, style)
                row_count += 1     
...
account_profit_loss_usd_xls(
        'report.account.profit.loss.usd.xls',
        'account.account',
