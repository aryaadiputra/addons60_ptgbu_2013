...
class eagle_contract( osv.osv ):
	...
	def get_view_selections(self, cr, uid, context={}):
		...
		lst += [('invDetails','Invoices Details')]
	...
	_columns = {
		...
		'activeTab_invDetails': fields.function( get_active_tabs, method=True, type='boolean', string="Active tab: Invoices Details" ),
	}
	...
	def get_invoice_line_default_values( self, cr, uid, invoice, contract, contract_position, context={} ):
		...
		vals.update( res['value'] )
		if contract_position and contract_position.notes:
			vals.update( { 'note': contract_position.notes } )
				...
				for contract_position in contract.positions:
					...
					if recurrence.unit == 'day':
						...
						if recurrence.value > 0:
							after -= relativedelta( days=1 )
					if before > now:
						netsvc.Logger().notifyChannel( 'addons.'+self._name, netsvc.LOG_DEBUG, "Skipping contract position "+str(contract_position.out_description)+" because of the dates: "+str(before)+" now, we are: "+str(now) )
						continue
