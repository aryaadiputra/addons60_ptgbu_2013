...
#            res.update({line.id:result})
    ...
    def _amount_current(self, cr, uid, ids, name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=None):
            ...
            date_end = line.order_id.date_order[:4]
            #acc_ids = line.budget_item_id.
            cr.execute("select sum(a.price_unit * (1-(a.discount/100.0)) * (1-(c.discount_total/100.0)) * a.product_qty) from purchase_order c, purchase_order_line a, budget_info_po b "
...
class purchase_order(osv.osv):
    ...
    def _get_approve_status(self, cr, uid, ids, field_name, arg, context=None):
        print "DDDDDDDDDDDDDDDDDDDDDDDDDDDD", ids
        result = {}
#        
#        STATE_SELECTION = [
...
#    ]
        ...
        if len(ids) <= 1:
            ...
            for po in self.browse(cr, uid, ids, context):
                if po.state == 'draft':
                    try:
                        to_approve_name = po.delegate.name or ""
                    except Exception,e:
                        to_approve_name = "No Purchase User Define"
                elif po.state == 'confirmed':
                    try:
                        employee_search = self.pool.get('hr.employee').search(cr, uid,[('user_id','=',po.delegate.id)])
                        employee_browse = self.pool.get('hr.employee').browse(cr, uid, employee_search)
                        
                        if employee_browse:
                            for employee in employee_browse:
                                to_approve_name = employee.department_id.manager_id.name
                        
                    except Exception,e:
                        to_approve_name = "No Procurement Manager Define"
                
                elif po.state == 'confirmed2':
                    try:
                        employee_search = self.pool.get('hr.employee').search(cr, uid,[('user_id','=',po.delegate.id)])
                        employee_browse = self.pool.get('hr.employee').browse(cr, uid, employee_search)
                        ...
                        if employee_browse:
                            for employee in employee_browse:
                                to_approve_name = employee.department_id.division_id.manager_id.name
                        
                    except Exception,e:
                        to_approve_name = "No Procurement Manager Define"
                        
                elif po.state == 'confirmed3':
                    try:
                        to_approve_name = po.mr_number.department.division_id.manager_id.name
                    except Exception,e:
                        to_approve_name = "No Head of Division Define"
                
                elif po.state == 'confirmed4':
                    to_approve_name = "Soebianto Hidayat"
                        
                
            print to_approve_name
            result[po.id] = to_approve_name or ""
        return result
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                ...
                print "OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO", line.price_subtotal
                for c in self.pool.get('account.tax').compute_all(cr, uid, line.taxes_id, (((line.price_unit * (1-(line.discount or 0.0)/100.0)) * (1-(line.order_id.discount_total or 0.0)/100.0))), line.product_qty, order.partner_address_id.id, line.product_id.id, order.partner_id)['taxes']:
    ...
    def create_date_div_req(self, cr, uid, ids):
        if self.check_budget_kadiv(cr, uid, ids):
            for po in self.browse(cr, uid, ids, context=None):
                if po.mr_number:
                    kadiv_id = po.mr_number.department.division_id.manager_id.user_id.id
                    print "kadiv_id :::", kadiv_id, "uid :::", uid
                    if kadiv_id <> uid and uid <> 1:
                        raise osv.except_osv(_('Please Insert Your Notes !'), _('Please Check Your ID.'))
    ...
    def check_budget_kadiv(self,cr, uid, ids, *args):
        print "OLAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA kadiv"
        #print "remain_budget------------------------------->>", remain_budget
        mr_total = 0.0
        for po in self.browse(cr, uid, ids):
            amount_total = po.amount_total
...
#            return True
    ...
    def check_budget_ceo(self,cr, uid, ids, *args):
        print "OLAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA CEO", ids
        #print "remain_budget------------------------------->>", remain_budget
...
class purchase_order_line(osv.osv):
    ...
    def _amount_line(self, cr, uid, ids, prop, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            taxes = tax_obj.compute_all(cr, uid, line.taxes_id, (((line.price_unit * (1-(line.discount or 0.0)/100.0)) * (1-(line.order_id.discount_total or 0.0)/100.0))), line.product_qty)
            print "taxes------------------->>", taxes
