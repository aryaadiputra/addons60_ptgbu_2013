<html>
	<head>
		...
		<style>
			...
			td.age{text-align:right;width:59px;}
			td.exp{text-align:right;width:100px;}
...
<center id="reportname" height="60 px">
	<h3 class="reportname">Employee List by Working Experience Time</h3>
</center
<br height="30px;"/>
<% 
datalist=data['form']
...
<table border="1" bordercolor="lightGrey" cellpadding="3" cellspacing="0" style="page-break-after:always;">
	<tr style="page-break-inside:avoid; page-break-after:auto;">
		...
		<td class="tabhead expl">Experience</td>
	</tr>
	<%count+=33%>
	%for wex in get_work_age():
		<% no=1 %>
		<tr style="page-break-inside:avoid; page-break-after:auto;">
			<td colspan="9" class="group">Working Experience Time : ${wex['name']}</td>
		</tr>
		<%count+=21%>
		<% employees=get_employee(datalist['active'],datalist['filter_dept'],datalist['filter_sect'],dummy,dummy,dummy,dummy,wex['value'],'doj DESC')%>
		%if employees>0:
			%for emp in employees:
				<tr style="page-break-inside:avoid; page-break-after:auto;">
					...
					<td class="data exp">${relativedelta (datetime.now(), datetime.strptime(emp.doj, '%Y-%m-%d')).years or "0"} years - ${relativedelta (datetime.now(), datetime.strptime(emp.doj, '%Y-%m-%d')).months or "0"} months</td>
				</tr>
				<% 
				count+=33
				no+=1
				...
				%if ((count+40)>900):
					...
					<table style="page-break-after:always;" border="1" bordercolor="lightGrey" cellpadding="3" cellspacing="0">
						<tr style="page-break-inside:avoid; page-break-after:auto;">
							...
							<td class="tabhead lvll">Level</td>
							<td class="tabhead wkpl">Workplace</td>
							<td class="tabhead expl">Experiences</td>
						</tr>
					<% count=30 %>			
				%endif
			%endfor
