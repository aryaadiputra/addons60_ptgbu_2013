...
from ad_account_optimization.report.account_balance import account_balance
import cStringIO
...
class account_balance_xls(report_xls):
    ...
    def generate_xls_report(self, parser, data, obj, wb):
        ...
        ws = wb.add_sheet(('Account Balance2 - %s - %s' % (c.partner_id.ref, c.currency_id.name))[:31])
        ws.panes_frozen = True
        ws.remove_splits = True
        ws.portrait = 0 # Landscape
        ws.fit_width_to_pages = 1
        judul = "ACCOUNT BALANCE REPORT"
        ...
        cols_specs = [
                ...
                ('Balance Init'   , 1, 80, 'number',
                    lambda x, d, p: x['balanceinit']),
                ('Debit'          , 1, 80, 'number',
                    lambda x, d, p: x['debit']),
                ('Credit'         , 1, 80, 'number',
                    lambda x, d, p: x['credit']),
                ('Balance'        , 1, 80, 'number',
                    lambda x, d, p: abs(x['balance'])),
                ('Balance Init FY', 1, 80, 'number',
                    lambda x, d, p: x['balanceinit_fy']),
                ('Debit FY'       , 1, 80, 'number',
                    lambda x, d, p: x['debit_fy']),
                ('Credit FY'      , 1, 80, 'number',
                    lambda x, d, p: x['credit_fy']),
                ('Balance FY'     , 1, 80, 'number',
                    lambda x, d, p: x['balance_fy']),
        ]
        ...
        tittle_style = xlwt.easyxf('font: height 240, name Arial Black, colour_index black, bold on; align: wrap on, vert centre, horiz center; pattern: pattern solid, fore_color gray25;')
        subtittle_left_style = xlwt.easyxf('font: name Arial, colour_index brown, bold on, italic on; align: wrap on, vert centre, horiz left; pattern: pattern solid, fore_color gray25;')
        ...
        for a in parser.lines(data['form']):
            if a['type'] == 'view':
...
account_balance_xls(
        'report.account.balance.full.xls',
        'account.account',
        'addons/ad_account_optimization/report/account_balance_full.rml',
