...
class new_contact_for_partner(osv.osv_memory):
	...
	_defaults = {
		...
		'partner_id': lambda self, cr, uid, ctx: ctx and 
			ctx.get('active_model','') == 'crm.lead' and 
			ctx.get('active_id',False) and
			self.pool.get('crm.lead').read(cr,uid,ctx['active_id'],['partner_id'],context=ctx)['partner_id']
			 or False,
	}
	...
	def make_contact(self, cr, uid, ids, context=None):
		...
		for data in self.browse(cr, uid, ids):
			...
			if data.partner_id:
				addrs = partner_obj.address_get( cr, uid, [data.partner_id.id] )
				if addrs and 'default' in addrs:
					job_id = job_obj.create(cr, uid, {
						'address_id': addrs['default'],
