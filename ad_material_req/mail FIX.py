    ...
    def check_mr(self, cr, uid, ids=False, context=None):
        ...
        for states in cr.fetchall():
            ...
            if state == 'draft':
                ...
                cr.execute("""select department_rel_employee from material_requisition where state = '%s' group by department_rel_employee"""% state)
            elif state in ('lv_2'):
...
#                return True
            for mr_res in cr.fetchall():
                ...
                if state == 'draft':
                    ...
                    mr_search = mr_obj.search(cr, uid, [('state','=',state),('department_rel_employee','=',res)])
                elif state in ('lv_2'):
...
#                    return True
                ...
                for mr in mr_browse:
                    ...
                    if state == 'draft':
                        ...
                        if mr.req_employee.department_id :
                            if mr.req_employee.department_id.manager_id:
                                email       = mr.req_employee.department_id.manager_id.user_id.user_email
                                mr_to       = mr.req_employee.department_id.manager_id.user_id.name
                        last_app    = mr.user_app
                    elif state == 'lv_2':
                        if mr.req_employee.department_id:
                            if mr.req_employee.department_id.division_id:
                                if mr.req_employee.department_id.division_id.manager_id:
                                    email       = mr.req_employee.department_id.division_id.manager_id.user_id.user_email
                                    mr_to       = mr.req_employee.department_id.division_id.manager_id.user_id.name
                    ...
                    mr_dept     = mr.department_rel_employee.name
                    mr_desc     = mr.origin
