    ...
    def _amount_all(self, cr, uid, ids, name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            ...
            if invoice.retention_check == True:
                res[invoice.id]['amount_total'] = res[invoice.id]['amount_tax'] + res[invoice.id]['amount_untaxed'] - invoice['amount_dp'] - retention_amount
            else:
                res[invoice.id]['amount_total'] = res[invoice.id]['amount_tax'] + res[invoice.id]['amount_untaxed'] - invoice['amount_dp']
           
#            for line in invoice.tax_line:
#                res[invoice.id]['to_be_paid'] = res[invoice.id]['amount_total'] - invoice['amount_dp']
            
            for line in invoice.invoice_line:
                ############################Remove Untuk DP pengurang AP################################
                #res[invoice.id]['to_be_paid'] = res[invoice.id]['amount_total'] - invoice['amount_dp']
                res[invoice.id]['to_be_paid'] = res[invoice.id]['amount_total']
            ####################################################
            
            res[invoice.id]['sub_total'] = res[invoice.id]['amount_ppn'] + res[invoice.id]['amount_untaxed']
            
        
#        tes = self.search(cr,uid,[('manual','=',False)])
#        hasil = self.browse(cr,uid,tes)
#            
#        for line in hasil.tax_line:
#            res[invoice.id]['amount_ppn'] += line.amount
        #abs(res[invoice.id]['holding_taxes'])
        print "res :", res
        return res
...
#            \n* The \'Cancelled\' state is used when user cancel invoice.'),
       # 'date_invoice': fields.date('Invoice Date', states={'paid':[('readonly',True)], 'open':[('readonly',True)], 'close':[('readonly',True)]}, select=True, help="Keep empty to use the current date"),
        ...
        'tax_line': fields.one2many('account.invoice.tax', 'invoice_id', 'Tax Lines22', readonly=True, states={'draft':[('readonly',False)]}),

        'move_id': fields.many2one('account.move', 'Journal Entry', readonly=True, select=1, ondelete='restrict', help="Link to the automatically generated Journal Items."),
        'amount_untaxed': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='DPP',
            store={
        ...
        'amount_ppn': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='VAT',
            store={
                'account.invoice': (lambda self, cr, uid, ids, c={}: ids, ['invoice_line'], 20),
                'account.invoice.tax': (_get_invoice_tax, None, 20),
                'account.invoice.line': (_get_invoice_line, ['price_unit','invoice_line_tax_id','quantity','discount','invoice_id'], 20),
        ...
        'sub_total': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='Total',
            store={
                'account.invoice': (lambda self, cr, uid, ids, c={}: ids, ['invoice_line'], 20),
                'account.invoice.tax': (_get_invoice_tax, None, 20),
                'account.invoice.line': (_get_invoice_line, ['price_unit','invoice_line_tax_id','quantity','discount','invoice_id'], 20),
        ...
        'holding_taxes': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='With Holding Taxes',
            store={
                'account.invoice': (lambda self, cr, uid, ids, c={}: ids, ['invoice_line'], 20),
                'account.invoice.tax': (_get_invoice_tax, None, 20),
                'account.invoice.line': (_get_invoice_line, ['price_unit','invoice_line_tax_id','quantity','discount','invoice_id'], 20),
        ...
        'to_be_paid': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='To be Paid',
            store={
                'account.invoice': (lambda self, cr, uid, ids, c={}: ids, ['invoice_line'], 20),
                'account.invoice.tax': (_get_invoice_tax, None, 20),
                'account.invoice.line': (_get_invoice_line, ['price_unit','invoice_line_tax_id','quantity','discount','invoice_id'], 20),
        ...
        'amount_tax': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='Tax',
            ...
            multi='all'),
        'amount_total': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Account'), string='Supplier Payable',
            store={
                'account.invoice': (lambda self, cr, uid, ids, c={}: ids, ['invoice_line'], 20),
                'account.invoice.tax': (_get_invoice_tax, None, 20),
                'account.invoice.line': (_get_invoice_line, ['price_unit','invoice_line_tax_id','quantity','discount','invoice_id'], 20),
        ...
        'check_total': fields.float('Supplier Payable', digits_compute=dp.get_precision('Account'), states={'open':[('readonly',True)],'close':[('readonly',True)]}),
    ...
    def copy(self, cr, uid, id, default={}, context=None):
        ...
        default.update({
            ...
            'move_name':False,
            'internal_number': False,
            'date_invoice_validation':False,
            'date_invoice_out':False,
            'date_due':False,
            'downpayment_id':False,
            'advance_id':False,
            'budget_info_ids_inv' : False,
        })
    ...
    def action_cancel(self, cr, uid, ids, *args):
        ...
        if move_ids:
            ...
            move_line = account_move_line_obj.search(cr, uid, [('move_id','=',move_ids)])
            ...
            if move_line:
                for l in move_line:
                    ...
                    if analytic_line_id:
                        print "11111111111111111111", analytic_line_id
