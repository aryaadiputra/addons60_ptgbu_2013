...
_fields = {
    'compare_item': {'string':'Ref. For % Comparing: ', 'type':'many2one', 'relation':'ad_budget.item'},
...
class wiz_budget_by_period(wizard.interface):
    ...
    def _build_form(self, cr, uid, data, context):
        """complete the form with abstracted parts from  ad_budget.wizard_abstraction """
        
        wiz_abstract_obj = pooler.get_pool(cr.dbname).get('ad_budget.wizard_abstraction')
    ...
    def _init_fields(self, cr, uid, data, context={}):
        ...
        budget_obj = pooler.get_pool(cr.dbname).get('ad_budget')
        
        res = {}

        #we come from budget
        if data['model'] == 'ad_budget':
        ...
        elif data['model'] == 'ad_budget.version':
            
            #from_date must match budget's start_date
            version_obj = pooler.get_pool(cr.dbname).get('ad_budget.version')            
        ...
        #we come from lines
            ...
            line_obj = pooler.get_pool(cr.dbname).get('ad_budget.line')
    ...
    states = {
        ...
        'print' : {
            ...
            'result' : {'type':'print', 'report':'budget_by_period2', 'get_id_from_action':True, 'state':'end'},
        },

    }
    
    
wiz_budget_by_period('ad_budget.by.periods')
