...
class account_balance_report(osv.osv_memory):
    ...
    _columns = {
        'type': fields.selection([('pdf','PDF111111'),('xls','Excel')], 'Type', required=True),
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        ...
        data['form'].update(self.read(cr, uid, ids, ['currency_rate', 'rate_opt'])[0])
        if wizard.type == 'xls':
            final_report = 'account.account.balance.xls'
        elif wizard.type == 'pdf':
            final_report = 'account.balancesheet.new'
#            if wizard.currency_rate:
#                final_report = 'account.trial.balance.pt.cur'
#            else:
