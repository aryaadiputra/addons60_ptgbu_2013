...
class budget_vs_reality(StandardReport):  
    ...
    def get_story3(self):
        ...
        if (self.datas['form']['split_by_dept']):
            print "Split by Dept"
        else:
            print "Dont Split by Dept"
        ...
        for v in versions: 
            ...
            for aa in aa_groups:
                ...
                table.add_num_column(v.name,'auto',2)
                table.add_num_column(self._('Actual'),'auto',2)
                table.add_num_column(v.name+" - "+self._('Actual'),'auto',2)
                ...
                for id in budget_values:
                    #print "budget VALUE", budget_values[id], "id=", id
                    #print "REAL VALUE", real_values[id], "id=", id
...
#                if can_compute:
    ...
    def _get_aa_children_and_consol(self, cr, uid, ids, context=None):
...
#                ids3.append(child.id)
    ...
    def _compute_budget_line_sum(self, cr, uid, ids, field_names, arg=None, context=None,
                  query='', query_params=()):
...
#                if can_compute:
    ...
    def get_story(self):
        ...
        #print "aa_groups ==========",aa_groups
        #for aa in aa_groups:
        #    print aa.department_id.id 
        #    print aa.department_id.name
        ...
        for v in versions: 
            ...
            for aa in aa_groups:
                ...
                #abi = budget_item_obj.compute_budget_sum(self.cr, self.uid, v.budget_id.budget_item_id.id)
                
                for line in lines:

                    budget_line_sum = self._compute_budget_line_sum(self.cr, self.uid, [line.id], [('balance_budget')], context=self.context)
                    
                    #print "line=",line.id
                
                print "aa.id=",aa.id
                
                #aa_budget_sum = self._compute_budget_sum(self.cr, self.uid, [aa.id], [('balance_budget')], context=self.context)
                #aa_real_sum = self._compute_real_sum(self.cr, self.uid, [aa.id], [('balance_real')], context=self.context)
                ...
                """
                        ...
                        #print i.code," | ",i.name," | ",budget_sum[i.id]['balance_budget']," | ",real_sum[i.id]['balance_real']
