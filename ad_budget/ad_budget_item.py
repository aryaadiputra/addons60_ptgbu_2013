    ...
    def __compute_real_sum(self, cr, uid, ids, field_names, arg=None, context=None,
        ...
        mapping = {
            'balance_real': "sum(credit) - sum(debit) as balance_real" ,
             #'balance_real': " sum(l.amount) as balance_real " ,
        }
        ...
        if children_and_consolidated:
            ...
            print "(tuple(periods),)", (tuple(periods),), len(periods)
            if len(periods) > 1:
            ...
            #################Sementara Saja##############################
            #filters = ' AND period_id in (%s) ' % tuple(periods)
            # 1 filters = ' AND period_id in %s ' % periods
...
#                       "   AND a.id = r.analytic_account_id " \
...
#                if can_compute:
    ...
    _columns = {
        ...
        'parent_id' : fields.many2one('ad_budget.item', 'Parent Item'),
        ...
        'type_budget' : fields.selection([('general', 'General'), ('cogs', 'COGS'), ('expense', 'OPEX'), ('capex', 'CAPEX')],'Type Budget'),
            }
    ...
    def get_real_values_from_analytic_accounts(self, cr, uid, item_id, periods,\
        ...
        budget_line_obj = self.pool.get('ad_budget.line')         
...
"""
    ...
    def _check_recursion(self, cr, uid, ids, context=None, parent=None):
        ...
        return super(ad_budget_item,self)._check_recursion(
                                                            cr,
                                                            uid,
                                                            ids,
    ...
    def search(self, cr, user, args, offset=0, \
        ...
        parent_result = super(ad_budget_item, self).search(cr, user, args, offset, limit, order, context, count)    
        #special search
        if context != None and 'budget_id' in context \
            and context['budget_id'] != False:
            
            budget = self.pool.get('ad_budget').browse(
                                                        cr, 
                                                        user, 
                                                        context['budget_id'], 
                                                        context
    ...
    def unlink(self, cr, uid, ids, context={}):
        ...
        try: 
            return super(ad_budget_item, self).unlink(
                                                        cr, 
                                                        uid, 
                                                        sub_ids, 
                                                        context
