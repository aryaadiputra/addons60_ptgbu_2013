...
class res_currency_dual(osv.osv):
    def _current_rate(self, cr, uid, ids, name, arg, context=None):
        ...
        for id in ids:
            cr.execute("SELECT currency_id, rate FROM res_currency_rate_dual WHERE currency_id = %s AND name <= %s ORDER BY name desc LIMIT 1" ,(id, date))
            if cr.rowcount:
    ...
    _columns = {
        ...
        'rate_ids': fields.one2many('res.currency.rate.dual', 'currency_id', 'Rates'),
    ...
    _defaults = {
        'active': lambda *a: 1,
        'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'res.currency.dual', context=c)
    }
    _order = "name"
    ...
    def read(self, cr, user, ids, fields=None, context=None, load='_classic_read'):
        ...
        for r in res:
            if r.__contains__('rate_ids'):
                rates=r['rate_ids']
                if rates:
                    currency_rate_obj=  self.pool.get('res.currency.rate.dual')
    ...
    def get_rate(self, cr, uid, from_currency_id, to_currency_id, context=None):
        ...
        rate = 1/self._get_conversion_rate(cr, uid, from_currency, to_currency, context)
...
class res_currency_rate_dual(osv.osv):
    _name = "res.currency.rate.dual"
    _description = "Currency Rate"
    _columns = {
        'name': fields.date('Date', required=True, select=True),
        'rate': fields.float('Rate', digits=(12,6), required=True,
            help='The rate of the currency to the currency of rate 1'),
        'currency_id': fields.many2one('res.currency.dual', 'Currency', readonly=True),
        'closing_rate'  : fields.boolean('Closing Rate'),
        'closing_rate_status': fields.selection([
                ('no',''),
                ('yes','Closing Rate'),
                ],'Closing Rate'),
                }
    
    def onchange_closing_rate(self, cr, uid, ids, date, closing_rate, context=None):
        res = {'value':{}}
        ...
        if closing_rate:
           res['value'] = {'closing_rate_status': 'yes'}
        else:
            res['value'] = {'closing_rate_status': 'no'}
        return res
