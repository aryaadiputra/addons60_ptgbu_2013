...
<center id="reportname" height="60 px">
	<h3 class="reportname">Employee List by Department</h3>
</center
<br height="30px;"/>
<% 
datalist=data['form'] 
...
<table border="1" bordercolor="lightGrey" cellpadding="3" cellspacing="0" style="page-break-after:always;">
	...
	%for dep in get_dept(datalist['filter_dept']):
		<% no=1 %>
		<tr style="page-break-inside:avoid; page-break-after:auto;">
			<td colspan="8" class="group">Departemen : ${dep.name}</td>
		</tr>
		<%count+=21%>
		<% employees=get_employee(datalist['active'],[dep.id],datalist['filter_sect'])%>
		%if employees>0:
