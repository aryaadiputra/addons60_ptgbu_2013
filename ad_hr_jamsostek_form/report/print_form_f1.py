...
class report_custom(report_int):
    ...
    def create(self, cr, uid, ids, datas, context=None):
        ...
        form = pool.get('form.f1').browse(cr,uid,ids[0])
        phone_list = [form.com_phone1, form.com_phone2]
        for pl in phone_list:
            if pl:
                if "-" not in pl:
                    raise osv.except_osv(_('Bad Format!'), _("Phone number must contained '-' ."))
...
#            result['com_contact_fax_area']      = faxarea
        ...
        try:
            ...
            try:
                fill_pdf(addons.get_module_resource('ad_hr_jamsostek_form','report','pdf','f1.pdf'), tmp_file, result)
...
report_custom('report.jamsostek.f1.print')

class form_f1(osv.osv):
    _inherit    = "form.f1"
    
    def print_form_f1(self, cr, uid, ids, context=None):
        active_ids = context.get('active_ids',[])
        data = {}
        data['form'] = {}
        data['ids'] = active_ids
        data['form']['name'] = self.browse(cr, uid, ids)[0].name
        return { 'type': 'ir.actions.report.xml', 'report_name': 'jamsostek.f1.print', 'datas': data, 'parser':form_f1}
