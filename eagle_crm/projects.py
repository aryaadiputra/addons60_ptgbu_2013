...
class project_issue(osv.osv):
	...
	def create( self, cr, uid, vals, context=None ):
		...
		if project_id:
			prj = projects.browse( cr, uid, project_id, context=context )
			if prj:
				if hasattr(prj, 'contract_id') and prj.contract_id:
					...
					if proj_ids and len( proj_ids ):
						...
						project = projects.browse( cr, uid, proj_ids[0], context=context )
						if project and project.analytic_account_id and project.analytic_account_id.partner_id:
							vals['partner_id'] = project.analytic_account_id.partner_id.id
					vals['contract_id'] = prj.contract_id.id
					if prj.contract_id.company_id:
						vals['company_id'] = prj.contract_id.company_id.id
	...
	def write( self, cr, uid, ids, vals, context=None ):
		...
		contract_id = vals.get('contract_id', False)
		if contract_id and contract_id.company_id:
			vals['company_id'] = prj.contract_id.company_id.id

		ret = super( project_issue, self ).write( cr, uid, ids, vals, context=context )
...
class task(osv.osv):
	...
	def write( self, cr, uid, ids, vals, context=None ):
		new_issue_state = False
		if 'state' in vals:
			if vals['state'] == 'done':
				new_issue_state = 'done'
			elif vals['state'] == 'cancelled':
				new_issue_state = 'cancel'
		
		ret = super( task, self ).write( cr, uid, ids, vals, context=context )

		if new_issue_state and ids and len(ids) > 0:
			issues_obj = self.pool.get( 'project.issue' )
			issues_ids = issues_obj.search( cr, uid, [('task_id','in',ids)], context=context )
			if issues_ids and len(issues_ids) > 0:
				issues_obj.write( cr, uid, issues_ids, { 'state': new_issue_state }, context=context ) 
