...
class budget_rolling_div(report_sxw.rml_parse):
    print "AAAAAAAAAAAAAAAAAAAAA"
    def __init__(self, cr, uid, name, context=None):
        super(budget_rolling_div, self).__init__(cr, uid, name, context=context)
        ...
        self.localcontext.update( {
            ...
            'get_total_row_capex' : self.get_total_row_capex,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(budget_rolling_div, self).set_context(objects, data, new_ids, report_type=report_type)    
    ...
    def get_total(self, as_of, fy, budget_item_id, type, dept=False, context=None):
        ...
        if type == 'view':
            ...
            for budget in budgets:
                ...
                cr.execute("Select id from hr_department where division_id=%s",(dept,))
                result_real_dept = cr.fetchall()
                if dept:
                    line_ids = db_pool.get('ad_budget.line').search(cr, uid, [('period_id','in',period_ids),('budget_item_id','=',budget.id),('analytic_account_id.department_id','in',result_real_dept)])
                    line_budget_ids = db_pool.get('ad_budget.line').search(cr, uid, [('period_id','in',period_budget_ids),('budget_item_id','=',budget.id),('analytic_account_id.department_id','in',result_real_dept)])
                else:
            ...
            result_real_dept = cr.fetchall()
            if dept:
                line_ids = db_pool.get('ad_budget.line').search(cr, uid, [('period_id','in',period_ids),('budget_item_id','=',budget_item_id),('analytic_account_id.department_id','in',result_real_dept)])
                line_budget_ids = db_pool.get('ad_budget.line').search(cr, uid, [('period_id','in',period_budget_ids),('budget_item_id','=',budget_item_id),('analytic_account_id.department_id','in',result_real_dept)])
            else:
    ...
    def get_total_BudgetD(self, as_of, fy, budget_item_id, type, dept=False, context=None):
        ...
        print "abs(total_view)", abs(total_view)
...
#       print period_ids,minimum,date_start
        ...
        for budget in budgets:
            ...
            if dept:
                line_ids = db_pool.get('ad_budget.line').search(cr, uid, [('period_id','=',period),('budget_item_id','=',budget.id),('analytic_account_id.department_id','in',result_real_dept)])
            else:
...
#                                total_view += line.amount 
    ...
    def get_total_row_capex(self, period, as_of, fy, budget_item_id, type, dept=False, context=None):
        print "CAPEXXXXXXXXXXXXXXXXXXXXXX"
...
#                            else:
    ...
    def get_department(self, data):
        ...
        print "obj_data[0]obj_data[0]obj_data[0]",obj_data[0]
        return dept
...
#        period_idx = period_pool.browse(self.cr, self.uid, period_ids)
        ...
        if type == 'view':
            ...
            for budget in budgets:
                ...
                #print "DEPT_DIV", dept
                print "DEPT*DEPT*DEPT*DEPT*DEPT*DEPT*", dept, dept<0
                
                print "222222222222222222222222222"
                print "result_real_dept--------------------->>", result_real_dept
                
                if dept:
                    #print "******************************************************", dept
                    line_ids = self.pool.get('ad_budget.line').search(self.cr, self.uid, [('period_id','=',period),('budget_item_id','=',budget.id),('analytic_account_id.department_id','in',result_real_dept)])
                else:
...
#                        #print "wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww", res[budget_item_id]
        ...
        for line in self.pool.get('ad_budget.line').browse(self.cr, self.uid, line_ids, context=context):
            ...
            date_from = line.period_id.date_start
            date_to = line.period_id.date_stop
            print "......",analytic_account_id, date_from, date_to, line.period_id
            cr.execute("SELECT SUM(amount) as balance_real FROM account_analytic_line WHERE account_id=%s AND (date "
                   "between to_date(%s,'yyyy-mm-dd') AND to_date(%s,'yyyy-mm-dd')) AND date <= %s ", (analytic_account_id, date_from, date_to, as_of,))
#            cr.execute("SELECT SUM(amount) as balance_real FROM account_analytic_line WHERE account_id=%s AND (date "
#                   "between to_date(%s,'yyyy-mm-dd') AND to_date(%s,'yyyy-mm-dd'))", (analytic_account_id, date_from, date_to))
...
#                    res[budget_item_id] = float(line.amount - line.balance_real)
    ...
    def get_transaction(self, budget_item, dept):
        ...
        cr.execute("Select id from hr_department where division_id=%s",(str(dept)))
        result_real_dept = cr.fetchall()
        
        analytic_search = budget_line.search(cr, uid, [('budget_item_id','=',budget_item),('analytic_account_id.department_id','in',result_real_dept)])
...
report_sxw.report_sxw('report.budget.rolling.pdf.div', 'ad_budget.item', 'addons/ad_budget_rolling_div/report/budget_rolling.mako', parser = budget_rolling_div)
