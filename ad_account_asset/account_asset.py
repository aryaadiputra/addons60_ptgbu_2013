...
class account_asset_category(osv.osv):
    ...
    _columns = {
        'code_extra': fields.char('Code', size=4, required=True),        
...
#            #########################################################
    ...
    def _amount_residual(self, cr, uid, ids, name, args, context=None):
        ...
        for asset1 in self.browse(cr, uid, ids, context):
            currency_id = asset1.currency_id.id
            
        cr.execute("""select l.asset_id as id, round(SUM(abs(l.amount))) As amount FROM account_asset_depreciation_line l 
                WHERE move_check = TRUE AND l.asset_id IN %s GROUP BY l.asset_id""", (tuple(ids),))
...
#        return res
    ...
    _columns = {
        ...
        'name': fields.char('Asset', size=64, ),
        ...
        #ADDED BY ADSOFT#
        'purchase_date_true': fields.date('Purchase Date', required=True, readonly=False, ),
        'purchase_date': fields.date('Used Date', required=True, readonly=False, ),
        
        #'maintenance_record': fields.one2many('account.asset.maintenance', 'responsible', 'Asset Maintenance', readonly=True, states={'draft':[('readonly', False)], 'open':[('readonly', False)]}),
        ...
        #additional by adsoft 
        'history_user': fields.one2many('account.asset.history.user', 'asset_id', 'History User', readonly=True, states={'draft':[('readonly', False)], 'open':[('readonly', False)]}),
        ...
        'amount_real_value': fields.float('Amount Purchase', required=False, readonly=True, states={'draft':[('readonly', False)]}), 
    }
    _defaults = {
        'code': lambda obj, cr, uid, context: obj.pool.get('ir.sequence').get(cr, uid, 'fa'),
        'purchase_date_true': lambda obj, cr, uid, context: time.strftime('%Y-%m-%d'),
    ...
    _sql_constraints = [
        ('name_uniq', 'unique(code)', 'Fixed Asset must be Unique !'),]
...
class account_move_line(osv.osv):
    ...
    _columns = {
        ...
        'name': fields.char('Name', size=268, required=True),
    }
account_move_line()
...
class account_asset_history_user(osv.osv):
     _name = 'account.asset.history.user'
     _description = 'For recorded user/employee asset'
     _columns = {
         'employee_id': fields.many2one('hr.employee', "User/Employee",),
     ...
     _defaults = {
         'dept_id': lambda s,cr,uid,c: s.pool.get('res.users').browse(cr,uid,uid).context_department_id.id,
      }
     
account_asset_history_user()

class account_asset_maintenance(osv.osv):
     _name = 'account.asset.maintenance'
     _description = 'For recorded maintenance Fix Asset'
     _columns = {
         'asset_id': fields.many2one('account.asset.asset', 'Asset', required=True),
         'responsible': fields.many2one('hr.employee', "User/Employee",),
         'start_date': fields.date("Start Date",),
