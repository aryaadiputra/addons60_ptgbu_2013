...
<center id="reportname" height="60 px">
	<h3 class="reportname">Employee List by Section</h3>
</center
<br height="30px;"/>
<% 
datalist=data['form'] 
...
<table border="1" bordercolor="lightGrey" cellpadding="3" cellspacing="0" style="page-break-after:always;">
	...
	<%count+=33%>
	%for section in get_section(dummy,datalist['filter_sect']):
		<% no=1 %>
		<tr style="page-break-inside:avoid; page-break-after:auto;">
			<td colspan="8" class="group">Section : ${section.name.title()}</td>
		</tr>
		<%count+=21%>
		<% employees=get_employee(datalist['active'],datalist['filter_dept'],[section.id])%>
		%if employees>0:
