...
class account_pl_report_opti(osv.osv_memory):
    ...
    _columns = {
        'type': fields.selection([('pdf','PDF333'),('xls','Excel')], 'Type', required=True),
        ...
        'currency_report': fields.selection([('idr','IDR'),('usd','USD')], 'Currency Report', required=True),
    }
    _defaults = {
        'type': lambda *a: 'pdf',
    }
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        ...
        if wizard.type == 'pdf':
            if wizard.currency_rate:
                ...
                final_report = 'ad.pl.account.new'
                #final_report = 'ad.pl.account'
        elif wizard.type == 'xls':
            if wizard.currency_report == 'usd':
                final_report = 'account.profit.loss.usd.xls'
            else:
                if wizard.new_report == True:
                    final_report = 'account.profit.loss.new.xls'
                else:
