...
class eagle_contract_pos(osv.osv):
	...
	def onchange_product(self, cr, uid, ids, product_id, qty, date_start, partner_id, pricelist_id):
		...
		context = {'lang': customer.lang, 'partner_id': partner_id} 
		...
		vals = self.onchange_description( cr, uid, ids, res['description'], res['recurrence_id'], res['next_invoice_date'], context=context )
...
class eagle_contract( osv.osv ):
	...
	def _get_user_id(self, cr, uid, cnt_ids, field_name, args, context={}):
		res = {}
		cr.execute( 'Select id,user_id from eagle_contract where id in %s', (tuple(cnt_ids),) )
	...
	_columns = {
		...
		'display_user_id': fields.function( _get_user_id, method=True, type='many2one', obj="res.users", string="Salesman", store=False, readonly=True ),
	...
	def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
		...
		if eagle_param and hasattr(eagle_param,'close_to_draft') and eagle_param.close_to_draft:
			...
			lst = res['arch'].split('\n')
			while (i < len(lst)):
				if 'contract_draft' in lst[i]:
					lst[i] = str(lst[i].replace('states="installation', 'states="installation,closed'))
					break
				i += 1
