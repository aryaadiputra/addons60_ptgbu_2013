...
#                for line in self.pool.get('ad_budget.line').browse(self.cr, self.uid, line_ids,):
#                    total_view += line.amount
#        print "abs(total_view)________________", abs(total_view)
#        return abs(total_view)
...
#                            res[budget_item_id] = res[budget_item_id] + amount
                        ...
                        #amount = float(line.amount - line.balance_real)
                            ...
                            print "period_id", period_id, "period", period
                            if period not in period_id:
                                amount = float(line.amount)
                            else:
                                amount = 0.00
...
#                    res[budget_item_id] = float(line.amount - line.balance_real)
                ...
                if line.amount <> 0.00:
                    #print "period",period
                    #print "result['balance_real']==========", result['balance_real']
                    if result['balance_real']:
                        amount = float(result['balance_real'])
                    #amount = float(line.amount - line.balance_real)
                    else:
                    ...
                    ###############Tambahan################
                    amount = float(result['balance_real'])
                    total = total + res[budget_item_id] + amount
                    ########################################
                    #return 0.0
        print "xxxxxTTTTTTTTT",total
        return total
...
#            budgets = budget_pool.browse(cr, uid, budget_id, context=ctx)
    ...
    def get_account_amount(self, data, i):
        #print "AAAAAAAAAAAAAAAAAAAAAA", datetime.strptime(data['form']['as_of'],'%Y-%m-%d').month
        ...
        cr.execute("""select id from account_period where date_start <= %s and date_stop >= %s 
                    """, (data['form']['as_of'],data['form']['as_of']))
        ...
        if analytic_browse:
            ...
            cr.execute("""
                select date_start, date_stop from account_period where fiscalyear_id = %s and date_start <= %s
                order by date_start
            ...
            seq = datetime.strptime(data['form']['as_of'],'%Y-%m-%d').month
            no  = 1
            query   = ""
            concat  = """ """
            ...
            for i in range_date_actual:
                ...
                if col_actual == no:
                    date_stop = str(data['form']['as_of'])
                ######################
                
                if no != 1:
                    #print "masuk"
                    query = """ UNION SELECT """+str(no)+""" as no,abs(SUM(amount)) as balance_real, """+str(col_actual)+""" as kolom FROM account_analytic_line 
                        WHERE account_id in """+str(analytic_account_id)+""" and date >= '"""+date_start+"""' and date <= '"""+date_stop+"""' """
                else:
                    query = """ SELECT """+str(no)+""" as no,abs(SUM(amount)) as balance_real, """+str(col_actual)+""" as kolom FROM account_analytic_line 
                        WHERE account_id in """+str(analytic_account_id)+""" and date >= '"""+date_start+"""' and date <= '"""+date_stop+"""' """
            ...
            cr.execute("""select id from account_period where date_start >= %s and fiscalyear_id = %s order by date_start
                    """,(data['form']['as_of'],data['form']['fiscalyear_id']))
            
            range_date_budget = cr.fetchall()
            #print "range_date_budget", range_date_budget
            if range_date_budget:
                for period_budget in range_date_budget:
                    #print "$$$$$$$$$$$$$$$$$$$$$$$$$"
                    period_budget = period_budget[0]
                    query_budget = """UNION select """+str(no)+""" as no, abs(SUM(amount)) as balance_real, """+str(col_actual)+""" as kolom from ad_budget_line where period_id = """ +str(period_budget)+ """
                            and analytic_account_id in """ +str(analytic_account_id)
                    
                    concat = concat + query_budget
                    
                    #cr.execute(query_budget)
                    #amount_budget = cr.fetchone()
                    no += 1
                    #print "amount_budget########################", amount_budget
            #print "data['form']['fiscalyear_id']", data['form']['fiscalyear_id']
            cr.execute("select id from account_period where fiscalyear_id = %s order by date_start", (data['form']['fiscalyear_id'],))            
            
            period_ids = tuple(map(lambda x: x[0], cr.fetchall()))
            #print "period_ids>>>>>>>>>>>>>>>>>>>>>>>", period_ids
            
            query_total_budget = """UNION select """+str(no)+""" as no, abs(sum(amount)) as balance_real, """+str(col_actual)+""" as kolom from ad_budget_line where period_id in """ +str(period_ids)+ """
                            and analytic_account_id in """ +str(analytic_account_id)
                    
            concat = concat + query_total_budget
            ######################################
