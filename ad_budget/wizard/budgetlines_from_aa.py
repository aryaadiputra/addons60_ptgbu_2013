...
class wiz_budgetlines_from_aa(wizard.interface):
    ...
    def _open_lines(self, cr, uid, data, context):
        ...
        query = """SELECT bl.id, p.date_start, p.date_stop 
                   FROM ad_budget_line bl, account_period p
    ...
    _create_fields = {
        'versions':    {'string':'Budgets Versions', 'type':'many2many', 'relation':'ad_budget.version'},
...
wiz_budgetlines_from_aa('budget.lines.from.aa')